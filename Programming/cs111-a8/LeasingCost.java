public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int nameIndex = description.indexOf(nameString) + 5;
        String newString1 = description.substring (nameIndex, description.length());
        int semiColon1 = newString1.indexOf(';');
        String name = newString1.substring(0, semiColon1);

        String typeString = "type:";
        int typeIndex = description.indexOf(typeString) + 5;
        String newString2 = description.substring (typeIndex, description.length());
        int semiColon2 = newString2.indexOf(';');
        String fuelType = newString2.substring(0, semiColon2);

        String dueString = "due:";
        int dueIndex = description.indexOf(dueString) + 4;
        String newString3 = description.substring (dueIndex, description.length());
        int semiColon3 = newString3.indexOf(';');
        String due = newString3.substring(0, semiColon3);
        double dueAtSigning = Double.parseDouble(due);

        String lengthString = "length:";
        int lengthIndex = description.indexOf(lengthString) + 7;
        String newString4 = description.substring (lengthIndex, description.length());
        int semiColon4 = newString4.indexOf(';');
        String length = newString4.substring(0, semiColon4);
        int leaseLength = Integer.parseInt(length);

        String monthlyString = "monthly:";
        int monthlyIndex = description.indexOf(monthlyString) + 8;
        String newString5 = description.substring (monthlyIndex, description.length());
        int semiColon5 = newString5.indexOf(';');
        String monthly = newString5.substring(0, semiColon5);
        double monthlyCost = Double.parseDouble(monthly);

        String mileUnitString = "mile/unit:";
        int mileUnitIndex = description.indexOf(mileUnitString) + 10;
        String newString6 = description.substring (mileUnitIndex, description.length());
        int semiColon6 = newString6.indexOf(';');
        String mileUnit = newString6.substring(0, semiColon6);
        double usage = Double.parseDouble(mileUnit);

        String allowanceString = "allowance:";
        int allowanceIndex = description.indexOf(allowanceString) + 10;
        String newString7 = description.substring (allowanceIndex, description.length());
        int semiColon7 = newString7.indexOf(';');
        String allowance = newString7.substring(0, semiColon7);
        int mileageAllowance = Integer.parseInt(allowance);

        double chargerCost = 0;
        if (fuelType.equals("electric")){
            String chargerString = "charger:";
            int chargerIndex = description.indexOf(chargerString) + 8;
            String newString8 = description.substring (chargerIndex, description.length());
            int semiColon8 = newString8.indexOf(';');
            String charger = newString8.substring(0, semiColon8);
            chargerCost = Double.parseDouble(charger);
        }
        ...
        // COMPLETE THIS METHOD
            for (Vehicle i : vehicles){
                if(i.getFuel().getType() == 2){
                    i.setCO2Emission(computeCO2(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION));
                    i.setFuelCost(computeFuelCost(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), electricityPrice));
                    computeLeaseCost(i.getLease().getDueAtSigning(), i.getLease().getLeaseLength(), i.getLease().getMonthlyCost());
                    double totalCost = computeFuelCost(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), electricityPrice) + computeLeaseCost(i.getLease().getDueAtSigning(), i.getLease().getLeaseLength(), i.getLease().getMonthlyCost()) + i.getFuel().getCharger();
                    i.setTotalCost(totalCost);
                }
                else{
                    i.setCO2Emission(computeCO2(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION));
                    i.setFuelCost(computeFuelCost(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), gasPrice));
                    computeLeaseCost(i.getLease().getDueAtSigning(), i.getLease().getLeaseLength(), i.getLease().getMonthlyCost());
                    double totalCost = computeFuelCost(i.getLease().getLeaseLength(), i.getFuel().getUsage(), i.getLease().getMileageAllowance(), gasPrice) + computeLeaseCost(i.getLease().getDueAtSigning(), i.getLease().getLeaseLength(), i.getLease().getMonthlyCost());
                    i.setTotalCost(totalCost);
                }
            }
	...
	public static void main (String[] args) {
        ...
        // a[0] = createVehicle("mile/unit:30; type:gas; name:330i; due:1500;	length:36; allowance:1500;monthly:123;");
        // computeCO2EmissionsAndCost(a, 3.85, 11.0);
        // StdOut.println(a[0].toString());
    }
}
